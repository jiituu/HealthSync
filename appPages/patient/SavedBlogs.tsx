...
import { BookOpen, ChevronLeft, ChevronRight, Search, X, Grid3X3, List, Filter } from "lucide-react"
...
type SortType = "latest" | "oldest" | "alphabetical" | "author"
...
const SavedBlogs: React.FC = () => {
  ...
  // Process and filter saved blogs
  const savedBlogs = savedBlogsData?.data || []

  // Filter blogs based on search term
  const filteredBlogs = savedBlogs.filter((blog: SingleBlogObject) => {
    ...
    return (
      ...
      `${blog.author?.firstname} ${blog.author?.lastname || ""}`.toLowerCase().includes(searchLower)
    )
  })
  ...
  return (
    <div className="mx-auto py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mb-6">Articles you have bookmarked for later reading</p>
        ...
        {debouncedSearchTerm && (
          <div className="mb-4">
            <p className="text-sm text-muted-foreground">
              {filteredBlogs.length === 0 ? "No results found for " : `Showing results for `}
              <span className="font-medium text-foreground">{debouncedSearchTerm}</span>
              <span className="text-muted-foreground"> in saved articles</span>
            </p>
          </div>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <Card className="p-8 text-center bg-muted/50">
          <h3 className="text-lg font-medium mb-2">No matching articles found</h3>
          <p className="text-muted-foreground">
            Try adjusting your search terms or clear the search to see all saved articles.
          </p>
...
const BlogCard: React.FC<BlogCardProps> = ({ blog, layout, onRemoveBookmark, onReadMore }) => {
