...
const Sidebar = ({ isDoctor }: SidebarProps) => {
  return (
    <div className="w-1/4 bg-gray-50 border-r border-gray-200 p-4 space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-800">{isDoctor ? 'Patients' : 'Doctors'}</h2>
      </div>

      <div className="space-y-4 overflow-auto" style={{ height: "calc(100vh - 80px)" }}>
        {patients.map((patient, index) => (
          <div key={index} className="flex items-center p-2 bg-gray-200 rounded-lg cursor-pointer">
            ...
            <span className="text-sm text-gray-800">{patient.name} <span className="text-xs text-gray-500">{patient.status}</span></span>
          </div>
        ))}
      </div>
    </div>
